...
(defvar ess-R-assign-ops-regexp 
  "\\(?:->\\|<\\(?:<?-\\)\\|\\(\\b\\s-*=\\s-*\\b\\)\\|:=\\)"
  )
;(defvar ess-R-assign-ops
;  (concat 
;    (regexp-opt list("<<-" "<-" "->") ; this was the intent
;                "\\b\\s-*=\\s-*\\b" "=:")
;    ); don't want "=" here which is not only for assign
  ;'("<<-" "<-" "->" "\\b\\s-*=\\s-*\\b" "=:") ; don't want "=" here which is not only for assign
;)
...
(defvar ess-R-numerical-regexp
  "[0-9]+\\(\\.[0-9]+\\)?"
  )

(defvar ess-R-constants-regexp
  (concat "\\<" "\\(" ess-R-numerical-regexp "\\|" 
          (regexp-opt ess-R-constants) "\\)" "\\>"
          )
  )

(defvar ess-R-delim-regexp
  (regexp-opt '("(" ")" "{" "}" "[" "]"))
  )

(defvar ess-R-operators-regexp
  (regexp-opt '("-" "+" "!" "~" "?" ":" "*" "/" "^" "%x%" "%%" "%/%" "%*%" "%o%" "%x%" "%in%"
      "<" ">" "==" ">=" "<=" "&" "&&" "|" "||" "$" "<-" "<<-" "->" "=" ))
  )
...
(defvar ess-extra-keywords-regexp
  (concat "\\<"
      (regexp-opt (read-lines "~/.emacs.d/R_words"))
      "\\>"
      )
  )
...
(defvar ess-R-common-font-lock-keywords
  (list
   ...
   (cons ess-R-operators-regexp ; all standard operators
	 'font-lock-reference-face)	; assign
;   (cons (concat "\\<" (regexp-opt ess-R-constants 'enc-paren) "\\>")
   (cons ess-R-constants-regexp
	 'font-lock-type-face)		; constants
;   (cons (concat "\\<" (regexp-opt ess-R-modifyiers 'enc-paren) "\\>")
;	 'font-lock-reference-face)	; modify search list or source ; screw the modifiers
   (cons ess-extra-keywords-regexp 
;	 (concat "\\(" font-lock-function-name-face "\\)") ) ; function name
	 'font-lock-function-name-face) ; function name
;   (cons ess-R-delim-regexp ; these don't work
;         '(1 font-lock-regexp-grouping-construct t)) ; also they aren't necessary
;   (cons ess-R-operators-regexp ;these don't work either
;         '(1 font-lock-negation-char-face t)) ; these might as well replace function
   )
